...
u32 XAdd_axi_Get_c(XAdd_axi *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XAdd_axi_ReadReg(InstancePtr->Ctrl_BaseAddress, XADD_AXI_CTRL_ADDR_C_DATA);
    return Data;
}

u32 XAdd_axi_Get_c_vld(XAdd_axi *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XAdd_axi_ReadReg(InstancePtr->Ctrl_BaseAddress, XADD_AXI_CTRL_ADDR_C_CTRL);
    return Data & 0x1;
}
