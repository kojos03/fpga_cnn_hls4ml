// ==============================================================
// Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2024.1.2 (64-bit)
// Tool Version Limit: 2024.05
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2024 Advanced Micro Devices, Inc. All Rights Reserved.
// 
// ==============================================================
#ifndef __linux__

#include "xstatus.h"
#ifdef SDT
#include "xparameters.h"
#endif
#include "xadd_axi.h"

extern XAdd_axi_Config XAdd_axi_ConfigTable[];

#ifdef SDT
XAdd_axi_Config *XAdd_axi_LookupConfig(UINTPTR BaseAddress) {
	XAdd_axi_Config *ConfigPtr = NULL;

	int Index;

	for (Index = (u32)0x0; XAdd_axi_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XAdd_axi_ConfigTable[Index].Ctrl_BaseAddress == BaseAddress) {
			ConfigPtr = &XAdd_axi_ConfigTable[Index];
			break;
		}
	}

	return ConfigPtr;
}

int XAdd_axi_Initialize(XAdd_axi *InstancePtr, UINTPTR BaseAddress) {
	XAdd_axi_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XAdd_axi_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
		InstancePtr->IsReady = 0;
		return (XST_DEVICE_NOT_FOUND);
	}

	return XAdd_axi_CfgInitialize(InstancePtr, ConfigPtr);
}
#else
XAdd_axi_Config *XAdd_axi_LookupConfig(u16 DeviceId) {
	XAdd_axi_Config *ConfigPtr = NULL;

	int Index;

	for (Index = 0; Index < XPAR_XADD_AXI_NUM_INSTANCES; Index++) {
		if (XAdd_axi_ConfigTable[Index].DeviceId == DeviceId) {
			ConfigPtr = &XAdd_axi_ConfigTable[Index];
			break;
		}
	}

	return ConfigPtr;
}

int XAdd_axi_Initialize(XAdd_axi *InstancePtr, u16 DeviceId) {
	XAdd_axi_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XAdd_axi_LookupConfig(DeviceId);
	if (ConfigPtr == NULL) {
		InstancePtr->IsReady = 0;
		return (XST_DEVICE_NOT_FOUND);
	}

	return XAdd_axi_CfgInitialize(InstancePtr, ConfigPtr);
}
#endif

#endif

